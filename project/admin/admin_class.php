...
Class Action {
	...
	function login_faculty(){
		...
		extract($_POST);		
		$qry = $this->db->query("SELECT *,title as name FROM faculty where id_no = '".$id_no."' ");
		if($qry->num_rows > 0){
			foreach ($qry->fetch_array() as $key => $value) {
				if($key != 'password' && !is_numeric($key))
					$_SESSION['login_'.$key] = $value;
	...
	function save_chair(){
		...
		$data .= ", description = '$description' ";
			if(empty($id)){
				$save = $this->db->query("INSERT INTO chairs set $data");
			}else{
				$save = $this->db->query("UPDATE chairs set $data where id = $id");
			}
		if($save)
			return 1;
	}
	function delete_chair(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM chairs where id = ".$id);
		if($delete){
			return 1;
		}
	}
	...
	function save_audience(){
		...
		$data .= ", type = '$type' ";
			if(empty($id)){
				$save = $this->db->query("INSERT INTO audiences set $data");
			}else{
				$save = $this->db->query("UPDATE audiences set $data where id = $id");
			}
		if($save)
			return 1;
	}
	function delete_audience(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM audiences where id = ".$id);
		if($delete){
			return 1;
		}
	}
